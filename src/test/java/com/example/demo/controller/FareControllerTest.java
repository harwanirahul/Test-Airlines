...
public class FareControllerTest {
	...
	public void testGetFareByFlightId() throws Exception
	{
		...
		MockHttpServletRequestBuilder requestBuilder= MockMvcRequestBuilders.get("/fare-by-flight-id")
				.param("id", id+"");
		MvcResult result = mockMvc.perform(requestBuilder).andExpect(status().isOk()).andReturn();
        System.out.println(result.getResponse().getContentAsString());
		assertTrue(result.getResponse().getContentAsString().contains("<h1>Welcome to ABC Airlines </h1> <h2>Here are the fare details for requested flightId: </h2"));
		assertTrue(result.getResponse().getContentAsString().contains("<h1>Welcome to ABC Airlines </h1>"));
		assertTrue(result.getResponse().getContentAsString().contains("<strong>Flight ID: </strong>"+getFlight().getId()));
	}
